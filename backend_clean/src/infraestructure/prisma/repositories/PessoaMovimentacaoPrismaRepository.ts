...
export class PessoaMovimentacaoPrismaRepository implements IPessoaMovimentacaoRepository {
    ...
    async findAll(): Promise<PessoaMovimentacao[]>{
        const pessoasMovimentacao = await prisma.pessoaMovimentacao.findMany();
        return pessoasMovimentacao.map((p: { id: string; idPessoa: string; idMovimentacao: string}) => new PessoaMovimentacao(
            p.id,p.idPessoa, p.idMovimentacao
        ));
